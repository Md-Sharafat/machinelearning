...
class CSVLoggerCallback(tf.keras.callbacks.Callback):
    def on_trial_end(self, trial):
        ...
        # Get the current best value so far
        best_trial = tuner.oracle.get_best_trials(num_trials=1)[0]
        best_hps = best_trial.hyperparameters.values
        best_val_loss = best_trial.score
        
        # Log current trial hyperparameters and best so far
        results.append({**hps, "val_loss": val_loss, **{f"best_{k}": v for k, v in best_hps.items()}, "best_val_loss": best_val_loss})

# Perform hyperparameter search
tuner.search(X_train, y_train, validation_data=(X_test, y_test), batch_size=32, epochs=30, callbacks=[CSVLoggerCallback()])
